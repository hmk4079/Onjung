const supportLayout = document.getElementById("contest-list");

// 메인 리스트 렌더링 함수
const showList = ({ supports, pagination }) => {
    console.log("showList 함수가 호출되었습니다.");
    console.log("supports 데이터:", supports);
    console.log("pagination 데이터:", pagination);
    let layText = ``;

    // 서버로부터 받은 supports가 비어있는지 확인
...
// 남은 기간 텍스트 설정
            ...
            // 리스트 항목 HTML 생성
            layText += `
                <a href="/support/support-inquiry/${support.id}" class="support-list-a">
                    <div class="contest-info">
                        <div class="contest-info-top">
                            <div class="outline-info">
                                <div class="outline-info-category">
                                    ...
                                    <span></span>${support.postTypeDisplayName || '타입없음'}
                                </div>
                                <div class="outline-info-title">
                                    <h2>${support.postTitle || '제목없음'}</h2>
                                </div>
                                <div class="outline-info-companyDesc">
                                    ${support.postSummary || '요약본없음'}
                                </div>
                            </div>
                        </div>
                        <div class="contest-info-bottom">
                            ...
                            <div class="contest-info-bottom-user-nick">
                                ${support.memberName != null ? support.memberName : (support.memberNickName != null ? support.memberNickName : '닉네임없음')}
                            </div>
                            ...
                            <div class="contest-info-bottom-created-date">
                                작성일 : ${support.createdDate || '작성일 없음'}
                            </div>
                        </div>
                    </div>
                    <ul class="prize-info">
                        <li class="prize-info-row">
                            ...
                            <div class="prize-info-row-label">
                                ${support.currentPoint + "포인트" || '후원금 없음'}
                            </div>
                        </li>
                        <li class="prize-info-row">
                            <div class="prize-info-row-key">
                            ...
                            <div class="prize-info-row-label">
                                ${support.goalPoint + "포인트" || '요청 후원금 없음'}
                            </div>
                        </li>
                        <li class="prize-info-row">
                            <div class="prize-info-row-key">
                        ...
                        <li class="prize-info-row">
                            ...
                            <div class="prize-info-row-label">
                                <div class="prize-info-row-label-date">
                                    ${support.supportSDate || '시작시간없음'} ~ ${support.supportEDate || '끝나는시간없음'} (24시까지)
                                </div>
                            </div>
                        </li>
                    </ul>
...
const addPageButtonEventListeners = () => {
    document.querySelectorAll(".page-btn").forEach(button => {
        button.addEventListener("click", (e) => {
            ...
            // 데이터 재로드
            fetchSupports(currentOrder, currentPage);
